...
public class GuiLogin extends GuiScreen
{
  ...
  public void initGui()
  {
    ...
    buttonList.add(new GuiButton(1, width / 2 - 100, height / 4 + 120 + 12, I18n.getString("gui.cancel")));
    ...
    if (label.equals("Register")) {
      textField = new GuiTextField(fontRenderer, width / 2 - 100, 60, 200, 20);
      textField.setFocused(true);
      textField.setText(savedPass != null ? savedPass : "");
      remember.setChecked((savedPass != null) && (!savedPass.isEmpty()));
    } else {
      passField = new GuiPasswordField(fontRenderer, width / 2 - 100, 60, 200, 20);
  ...
  protected void keyTyped(char par1, int par2)
  {
    if (label.equals("Register")) {
      ...
      ((GuiButton)buttonList.get(0)).enabled = (textField.getText().trim().length() > 0);
    } else {
  ...
  public void drawScreen(int par1, int par2, float par3)
  {
    ...
    drawCenteredString(fontRenderer, label, width / 2, 20, 16777215);
    drawString(fontRenderer, "This server is asking you to " + label.toLowerCase(), width / 2 - 100, 47, 10526880);
    drawString(fontRenderer, "Remember ?", width / 2 - 100, 90, 10526880);
    if (label.equals("Register")) {
